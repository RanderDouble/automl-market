...
| 机制改进 | 强制购买口径、冷启动、搜索成本和发现--定价割裂如何修正 | 比较外部选项修正、有限场景鲁棒、成本感知 Markov 定价和 warm-start Pricing-guided Data-Bandit | `results/figures/improvement_*.pdf` |
...
6. `scripts/run_improvement_experiments.py`：怎样生成机制改进表格和图。
...
如果不想手动激活环境，也可以通过 `conda run` 执行：

```bash
conda run -n automl-market make all
```

`make all` 的执行顺序是：20 项单元测试；旧版补充实验；机制改进实验；RQ1；RQ2/RQ3；中文报告；统一展示；RQ1/RQ3 讲义；论文中文摘要。默认实验固定随机种子，`environment.yml` 也锁定了影响数值和图形的 NumPy、Matplotlib 与 HiGHS 版本，因此应得到相同 CSV/JSON 和图。
...
- 通过最优停止 DP 在 `(历史最佳质量, 当前质量)` 状态上做精确前向传播，计算每类买家的停止时间似然。
...
在 `environment.yml` 对应的 Conda 环境中，随机先验、批量 100 时，最终 KL 应约为 `0.12083`、`0.01926`、`0.00495`；对应尾部波动约为 `0.00015`、`0.00062`、`0.00387`。因此常数 `1/2` 快但持续振荡，`1/(t+1)` 很稳但早期衰减过快，`1/sqrt(t)` 是较均衡的折中。核验时应使用项目 Conda 环境，并以 CSV 为准。
...
机制改进实验不依赖原文未公开数据，全部使用项目内透明合成环境。运行：
...
- Robust IR；
- Cost-aware Markov；
- Robust cost-aware Markov。

核心指标：

- `markov_nominal_cost_003`：真实 Markov 停止行为、名义先验、搜索成本 `c=0.03` 下收入；
- `markov_oos_prior_cost_003`：OOS 先验、搜索成本 `c=0.03` 下收入；
- `markov_worst_prior_cost`：多个先验和多个搜索成本场景上的最坏情况收入。

当前主实例结果应显示：Cost-aware Markov 在名义 `c=0.03` 和 OOS 先验 `c=0.03` 下最高，分别为 `0.3082` 和 `0.2231`；Robust cost-aware Markov 的最坏情况收入最高，为 `0.1857`。5 个扰动市场上的均值应显示：Cost-aware Markov 的名义收入最高，为 `0.3072±0.0116`；Robust cost-aware Markov 的最坏情况收入最高，为 `0.1843±0.0033`。

发现部分比较原 RQ1 四种方法和 `Pricing-guided Data-Bandit`。当前主分数表结果应显示：Pricing-guided Data-Bandit 的 normalized AUC 为 `0.7583`，高于 Data-Bandit 的 `0.6949`；前 4 次训练平均效用为 `0.7350`，高于 Data-Bandit 的 `0.5415`。12 个扰动分数表上的价格信号消融应显示：Pricing-guided Data-Bandit 的 AUC 为 `0.7764±0.0070`，高于 Random-signal guided 的 `0.7588±0.0081`，但随机信号本身也带来明显排序收益，因此价格信号应解释为 warm-start 信息的有限增量。

核对文件：

- `results/improvement_experiments_summary.json`：改进实验参数和结果；
- `results/tables/improvement_pricing_results.csv`：定价改进表；
- `results/tables/improvement_pricing_multi_instance.csv`：5 个扰动市场的定价改进均值与标准误；
- `results/tables/improvement_pricing_scenarios.csv`：鲁棒先验与搜索成本场景；
- `results/tables/improvement_discovery_results.csv`：发现改进表；
- `results/tables/improvement_discovery_signal_ablation.csv`：12 个扰动分数表上的价格信号消融；
- `results/tables/improvement_summary.csv`：每组实验主指标最佳方法；
- `results/figures/improvement_pricing_revenue.pdf`：定价改进图；
- `results/figures/improvement_pricing_multi_instance.pdf`：多实例定价改进图；
- `results/figures/improvement_discovery_curves.pdf`：发现改进图；
- `results/figures/improvement_discovery_signal_ablation.pdf`：价格信号消融图；
- `docs/improvement_results.md`：可直接引用的改进实验说明。
...
连续 MILP 与估值网格差异，以及较大 MILP 的 60 秒求解间隙都保留在 `results/summary.json`、`results/tables/pricing_results.csv` 和报告补充章节中。它们用于说明求解器边界，不属于当前 RQ1/RQ2/RQ3 复现主线。本阶段不继续增加商业求解器调参或转移矩阵误差大范围消融。
