...
- 通过最优停止 DP 的精确前向状态传播计算每类买家的停止时间似然，不再使用有限条 Markov 轨迹进行 Monte Carlo 估计。
...
随机先验、批量 100 时，最终 KL 应约为 `0.05185`、`0.00798`、`0.00702`；对应尾部波动约为 `0.00019`、`0.00032`、`0.00358`。因此常数 `1/2` 快但持续振荡，`1/(t+1)` 很稳但早期衰减过快，`1/sqrt(t)` 是较均衡的折中。
