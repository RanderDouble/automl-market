...
\title{机器学习数据获取与数据定价}
\subtitle{Optimal Pricing for Data-Augmented AutoML Marketplaces\\复现、问题与改进}
\author{课程大作业 C.3｜中期展示}
\date{2026年7月｜项目版本 v0.2.0}
...
\begin{itemize}
  ...
  \item Markov 性能轨迹模拟
  \item 论文强制选择经验目标
  \item 2401 条候选价格曲线精确搜索
  \item Independent pricing 基线
  \item 停止时间贝叶斯先验学习
  \item IR-aware 与 Robust IR 改进
  \item 测试、图表、JSON/CSV、9 页报告
\end{itemize}
\end{exampleblock}
\column{0.49\textwidth}
\begin{block}{当前实验设置}
\begin{itemize}
  \item $|\mathcal Q|=4,|\Theta|=6,T=6$
  \item 240 条训练轨迹
  \item 4000 条测试轨迹
  \item IS：训练同分布
  \item OOS：先验偏向低支付意愿类型
  \item 固定种子：20260716
  \item Git：\texttt{main}，版本化结果可追踪
\end{itemize}
\end{block}
\end{columns}
\end{frame}

\begin{frame}{复现结果：定价与先验学习}
\begin{columns}[T,onlytextwidth]
\column{0.55\textwidth}
\includegraphics[width=\linewidth]{pricing_comparison.pdf}
\column{0.43\textwidth}
\includegraphics[width=\linewidth]{prior_learning.pdf}
\vspace{-0.4em}
\begin{block}{观察}
\small
\begin{itemize}
  \item IR-aware：IS 测试收入最高
  \item Robust IR：OOS 收入最高
  \item 先验学习 KL：$0.0311\to0.0069$
  \item 停止时间有信息，但类型可识别性不是自动成立
\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item 尚未完成真实数据上的 Exp3 RQ1
  \item 单随机种子不足以给出统计显著性
  \item 小网格实验不能替代 $|Q|=20$ 的 MILP
\end{itemize}
\end{alertblock}
\end{columns}
\end{frame}

\begin{frame}{后续计划与最终交付}
\begin{tikzpicture}[
...
\node[milestone,right=of m1] (m2) {v0.3\\公开表格数据 + Exp3 / RQ1};
\node[milestone,right=of m2] (m3) {v0.4\\MILP $|Q|=20$ + 多种子实验};
\node[milestone,right=of m3] (m4) {v1.0\\最终报告、代码、复现说明};
\draw[arrow] (m1)--(m2);
\draw[arrow] (m2)--(m3);
\draw[arrow] (m3)--(m4);
\end{tikzpicture}
...
\vspace{1.2em}
\begin{enumerate}
  \item 实现简化 augmentation--model Exp3，完成 RQ1；
  \item 接入 Gurobi/SCIP，在论文规模上求解原 MILP 与 IR-MILP；
  \item 运行 20--50 个随机种子，加入均值、标准差和显著性分析；
  \item 对先验偏移、发现成本和 $P$ 的估计误差做消融；
  \item 整合成最终 PDF、实验代码、环境与一键复现说明。
\end{enumerate}
\end{frame}
...
\end{block}
\begin{exampleblock}{我们的阶段贡献}
\begin{enumerate}
  \item 实现并验证论文的停止 DP、经验定价与先验学习；
  \item 发现“自愿购买 vs. 强制选择”的建模缺口；
  \item 给出 IR 外部选项的理论修正；
  \item 用鲁棒 IR 提升先验偏移下的收入稳定性；
  \item 建立可复现、可测试、可版本化的代码与报告工程。
\end{enumerate}
\end{exampleblock}
\vfill
...
\begin{itemize}
  ...
  \item 原始结果：\texttt{results/summary.json}
  \item 表格结果：\texttt{results/tables/pricing\_results.csv}
  \item 阶段报告：\texttt{report/main.pdf}
  \item 中期展示：\texttt{slides/midterm.pdf}
  \item 当前版本：\texttt{v0.2.0}
\end{itemize}
\vspace{0.8em}
\small
